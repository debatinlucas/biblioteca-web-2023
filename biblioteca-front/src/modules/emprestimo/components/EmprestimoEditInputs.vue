...
export default {
  ...
  methods: {
    ...
    onLivroChange(livroId, index) {
      this.emprestimoEditVm.emprestimo.livro_ids.forEach(livro => {
        if (livro === livroId.id) {
          this.livroErro = true;
      ...
      if (!this.livroErro) {
        this.emprestimoEditVm.emprestimo.livro_ids.push(livroId.id);
      }
    ...
    onRemoverLivros(index) {
      ...
      this.emprestimoEditVm.emprestimo.livro_ids.splice(index, 1);
    },
  },
};
</script>
